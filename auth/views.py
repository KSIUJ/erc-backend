...
from registry.models import Client, Member, Role, AuthEvent
...
def authorize(request):
    ...
    if DISCORD_ID in rq:
        try:
            return check_access(Member.objects.get(card_id=rq[DISCORD_ID]), client, AuthEvent.DISCORD, rq[DISCORD_ID])
        except Member.DoesNotExist:
            log_event(client, None, AuthEvent.DISCORD, rq[DISCORD_ID], False)
            return HttpResponseNotFound("Unknown discord id")
    if LDAP in rq:
        try:
            return check_access(Member.objects.get(card_id=rq[LDAP]), client, AuthEvent.LDAP, rq[LDAP])
        except Member.DoesNotExist:
            log_event(client, None, AuthEvent.LDAP, rq[LDAP], False)
            return HttpResponseNotFound("Unknown ldap uid")
    if EMAIL in rq:
        try:
            return check_access(Member.objects.get(card_id=rq[EMAIL]), client, AuthEvent.EMAIL, rq[EMAIL])
        except Member.DoesNotExist:
